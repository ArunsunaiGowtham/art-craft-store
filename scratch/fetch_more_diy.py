...
headers = {'User-Agent': 'ArtStoreDIYKitsFetcher/1.0 (contact@artcraft.org)'}

queries = [
    "mosaic kit craft",
    "glass tiles mosaic craft",
    "leathercraft tools needles",
    "pottery sculpting tools set",
    "resin crafting molds"
]
                ...
                with urllib.request.urlopen(req2, timeout=10) as res2:
                    ...
                    for pid, pinfo in pages.items():
                        if 'imageinfo' in pinfo:
                            ...
                            mime = ii.get('mime', '')
                            size = ii.get('size', 0)
                            if 'image' in mime and size > 80000 and ('jpg' in url.lower() or 'jpeg' in url.lower() or 'png' in url.lower()):
