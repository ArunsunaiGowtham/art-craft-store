...
url = "https://en.wikipedia.org/w/api.php?action=query&titles=Pencil&generator=images&gimlimit=25&prop=imageinfo&iiprop=url&format=json"
...
for k, v in pages.items():
    if 'imageinfo' in v and len(v['imageinfo']) > 0:
        ...
        if u.endswith('.jpg') or u.endswith('.png') or u.endswith('.jpeg'):
            try:
                time.sleep(0.5)
                req_img = urllib.request.Request(u, headers={'User-Agent': 'ArtCraftStore/4.0'})
                img_data = urllib.request.urlopen(req_img, context=ctx, timeout=8).read()
                clean_name = title.replace('File:', '').replace(' ', '_')[:30]
                out_path = f"scratch/pencil_{clean_name}.jpg"
