...
os.makedirs('scratch/flickr_clay', exist_ok=True)

queries = [
    ("modeling_clay", "modeling clay"),
    ("plasticine", "plasticine colors"),
    ("polymer_clay", "polymer clay"),
    ("air_dry_clay", "air dry clay"),
    ("fimo", "fimo clay"),
    ("pottery_clay", "pottery clay")
]
...
for tag, q in queries:
    ...
    for i, u in enumerate(urls[:5]):
        dest = f"scratch/flickr_clay/{tag}_{i+1}.jpg"
        try:
            download_img(u, dest)
            im = Image.open(dest)
